...
void FVM::NeumannCondition::apply(SparseMatrixDIA& A, Vectorb& b, size_t index, const ScalarCellField& phi) const{
    ...
    if (mesh.getdeltay_s(index) == 0) // Sud.
        b.addValue(index,-gamma_*value_*mesh.getDeltax(index));
    else if (mesh.getdeltay_n(index) == 0) // Nord.
        b.addValue(index,-gamma_*value_*mesh.getDeltax(index));
    else if (mesh.getdeltax_w(index) == 0) // Ouest.
        b.addValue(index,-gamma_*value_*mesh.getDeltay(index));
